...
def chatbot():
    ...
    speak("Hello! How are you?", tts_engine)
    ...
    while True:
        ...
        print(f"Wuzzu: {response}")
        speak(response, tts_engine)

        speak("Do you want to ask more?", tts_engine)
        more_input = recognize_speech()
        if "no" in more_input.lower():
            speak("Okay, see you next time!", tts_engine)
            break
